...
df_movies = read.csv("data/movie_titles.csv",  col.names= c("MovieID", "MovieRelease", "Title", "Title2", "Title3", "Title4"), fill=TRUE, sep = ",")

df_movies = unite(df_movies, Title, Title, Title2, Title3, Title4, sep = "")
...
#Añadir la media, la desviación típica, la moda y la mediana.
```
```{r}
library(reticulate)
...
aa =netflix_py.loc[,"MovieID"]
print(aa)

a = netflix_py.describe()
...
#boxplot(MovieID~Rating, data=mas_votadas)

#ggplot(data = mas_votadas, mapping = aes(x=MovieID, y=Rating, group=MovieID)) +
#  geom_boxplot()

ggplot(mas_votadas,aes(Title, Rating, group=MovieID))+
  geom_boxplot(fill = "darkseagreen1", colour = "black", outlier.colour = "darkgreen")+
   geom_point(stat= "summary", fun.y=mean, shape=20, size=3, color="red") 
```
